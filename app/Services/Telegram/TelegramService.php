...
class TelegramService
{
     ...
     */
    public function downloadFile($telegram_file_path, $local_file_path): bool
    {
        $file_url = 'https://api.telegram.org/file/bot' . $this->bot_token . '/' . $telegram_file_path;
        Log::info('Attempting to download from: ' . $file_url);
        ...
        // Use sendAPIRequest to download file content (with proxy)
        $file_content = $this->sendAPIRequest($file_url, [], false);
        if ($file_content === false) {
            Log::error('Download failed: Unable to fetch file content from ' . $file_url);
            return false;
        }

        // Save the file
        $result = file_put_contents($local_file_path, $file_content);
        if ($result === false) {
            Log::error('Failed to save file to: ' . $local_file_path);
            return false;
        }

        Log::info('File saved to: ' . $local_file_path);
        return true;
    }
    ...
    /*private function sendAPIRequest($url, array $content, $post = true)
    {
        ...
        if ($this->log_errors) {
            if (class_exists('TelegramErrorLogger')) {
                ...
                Log::warning(json_decode($result, true));
            }
        }
    ...
    private function sendAPIRequest($url, array $content, $post = true): string
    {
        // Append chat_id to URL if present
        if (isset($content['chat_id'])) {
            $url .= '?chat_id=' . urlencode($content['chat_id']);
            unset($content['chat_id']);
        }

        // Configure HTTP client
        $http = Http::withOptions([
            'verify' => false, // Disable SSL verification (same as CURLOPT_SSL_VERIFYPEER)
        ]);

        // Apply proxy settings if available
        if (!empty($this->proxy) && array_key_exists('url', $this->proxy) && array_key_exists('port', $this->proxy)) {
            $proxyUrl = $this->proxy['url'] . ':' . $this->proxy['port'];

            // Add authentication if available
            if (array_key_exists('auth', $this->proxy)) {
                $proxyUrl = $this->proxy['auth'] . '@' . $proxyUrl;
            }

            // Determine proxy type (http or socks5)
            $proxyScheme = 'http://'; // Default to HTTP
            if (array_key_exists('type', $this->proxy) && $this->proxy['type'] === CURLPROXY_SOCKS5) {
                $proxyScheme = 'socks5://';
            }

            // Set proxy in Laravel Http format
            $http = $http->withOptions([
                'proxy' => $proxyScheme . $proxyUrl,
            ]);
        ...
        try {
            ...
            if (!$response->successful()) {
                $result = json_encode([
                    'ok' => false,
                    'curl_error_code' => $response->status(),
                    'curl_error' => $response->reason(),
                ]);
            }
        } catch (\Exception $e) {
            ...
            Log::warning('Error message :' . json_encode($loggerArray));
        }
